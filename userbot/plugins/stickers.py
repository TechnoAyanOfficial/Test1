...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "AaryanBot"
...
async def _(event):
    ...
    #packname = f"TheRajput"
    #packshortname = f"AaryanBot_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1025131428:
        packname = f"AaryanStickers"
        packshortname = "Aaryan_Property"
    else:
        packname = f"AaryanBots{userid}"
        packshortname = f"AaryanBots_{userid}_ns"
    await event.edit("`Look dat way,it's a gurl!\nMeanwhile, lemme Buy this sticker over here ヽ༼ ಠ益ಠ ༽ﾉ`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "AaryanBot_Sticker.png"
    ...
    if is_a_s:
        ...
        if userid == 1025131428:
            packshortname = "Aaryan_Animated"
        else:
            packshortname = f"AaryanBot_{userid}_an" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    pack += 1
                    prevv = int(pack) - 1
                    packname = f"{user.first_name}'s AaryanBot Vol.{pack}"
                    packshortname = f"Vol_{pack}_with_{user.first_name}"
                    #if userid == 1025131428:
                       # packname = f"{user.first_name}'s AaryanBot Vol.{pack}"
                       # packshortname = "Vol._{pack}_Aaryan_Sticker"
                   # else:
                       # packname = f"Vol._{pack}_AaryanBot{userid}"
                        #packshortname = f"Vol._{pack}_AaryanBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        file_caption = "https://t.me/Therajput"
        if is_a_s:
            file_ext_ns_ion = "tgs"
